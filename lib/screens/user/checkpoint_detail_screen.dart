      ...
      if (diffMin < 3) {
        ...
        out.add(
          _TimelineEntry(
            ...
            footNote: arabicRelativeSince(t) == 'الآن' ? 'الآن' : null,
          ),
        );
      } else {
        if (te.isAfter(tx)) {
...
class _SendUpdatePanelState extends State<_SendUpdatePanel> {
  ...
  Future<void> _submit(BuildContext context) async {
    ...
    try {
      final List<String> tagSnapshot = CheckpointReportTag.normalizeList(_tags);
      await provider.updateStatus(
        c.id,
        'entrance',
        _entranceOption.repoStatus,
        tags: tagSnapshot,
      );
      await provider.updateStatus(
        c.id,
        'exit',
        _exitOption.repoStatus,
        tags: tagSnapshot,
      );
      if (context.mounted) {
